...
internal class InvalidationTrackableTransformer(
  ...
  private val delegation = InvalidationTrackableTransformerDelegation(context, logger, stabilityInferencer)
  ...
  override fun visitSimpleFunction(declaration: IrSimpleFunction): IrStatement {
    ...
    return super.visitSimpleFunction(delegation.lower(declaration, currentInvalidationTrackTable!!))
  }
}

private class InvalidationTrackableTransformerDelegation(
  private val context: IrPluginContext,
  private val logger: VerboseLogger,
  ...
  private var currentFunctionScope: Scope? = null
  private var currentInvalidationTrackTable: IrInvalidationTrackTable? = null

  private fun irTmpVariableInCurrentFun(expression: IrExpression, nameHint: String? = null): IrVariable =
    currentFunctionScope!!.createTemporaryVariable(irExpression = expression, nameHint = nameHint)

  fun lower(function: IrSimpleFunction, invalidationTrackTable: IrInvalidationTrackTable): IrSimpleFunction {
    currentFunction = function
    currentFunctionScope = Scope(function.symbol)
    currentInvalidationTrackTable = invalidationTrackTable

    return function.apply { body?.transformChildrenVoid() }
  }
  ...
  override fun visitBlock(expression: IrBlock): IrExpression {
    ...
    val validValueParamters = currentFunction.valueParameters.dropLast(2)
    ...
    val currentFunctionKeyName = irTracee[DurableWritableSlices.DURABLE_FUNCTION_KEY, currentFunction]!!.name
    val computeDiffParamsIfPresent =
      currentInvalidationTrackTable.irComputeDiffParamsIfPresent(
        ...
        originalName = irString(currentFunction.name.asString()),
        paramInfos = paramInfos,
      )
    val computeDiffParamsIfPresentVariable = irTmpVariableInCurrentFun(
      computeDiffParamsIfPresent,
      nameHint = "${currentFunction.name.asString()}\$diffParams",
    )
    ...
    val affectedComposableCall = IrConstructorCallImpl.fromSymbolOwner(
      ...
      putValueArgument(0, irString(currentFunction.name.asString()))
      putValueArgument(1, irString(currentFunction.kotlinFqName.asString()))
    }
  ...
  private fun getCurrentFunctionNameIntercepttedAnonymous(): String {
    val currentFunction = currentFunction!!
    val currentFunctionName = currentFunction.name
    return if (currentFunctionName == SpecialNames.ANONYMOUS) {
      try {
        val parent = currentFunction.parent
