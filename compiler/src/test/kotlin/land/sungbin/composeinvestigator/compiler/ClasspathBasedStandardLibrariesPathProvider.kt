...
object ClasspathBasedStandardLibrariesPathProvider : KotlinStandardLibrariesPathProvider() {
  ...
  private val jars =
    System.getProperty("java.class.path")
      ...
      .map(::File)
      .associateBy { GRADLE_DEPENDENCY.matchEntire(it.path)?.groups["name"]?.value ?: it.name }

  private fun getFile(name: String): File =
    jars[name] ?: error("Jar $name not found in classpath:\n${jars.entries.joinToString("\n")}")
  ...
  override fun scriptingPluginFilesForTests(): Collection<File> = TODO("KT-67573")
}
