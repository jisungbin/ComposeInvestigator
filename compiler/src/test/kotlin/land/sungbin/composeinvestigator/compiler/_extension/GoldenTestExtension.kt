...
typealias VerifyGolden = (ir: String, source: String) -> GoldenVerificationResult
...
class GoldenTestExtension : BeforeEachCallback, ParameterResolver {
  ...
  fun verifyGolden(ir: String, source: String): GoldenVerificationResult =
    verifyGolden(File(DEFAULT_GOLDEN_DIRECTORY, goldenPath).path, ir, source)
  ...
  companion object {
     ...
     */
    fun verifyGolden(goldenPath: String, ir: String, source: String): GoldenVerificationResult {
      val golden = File(goldenPath)
      if (!golden.exists()) {
        saveGolden(golden, GoldenTransformTestInfo(ir, source))
